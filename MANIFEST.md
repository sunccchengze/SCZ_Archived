...
> 所有仓均保留原仓历史与默认分支；本仓只含内容并集快照 + `README.ARCHIVE.md`。
...
| `hogwarts-sorting-hat-quiz` | `hogwarts-sorting-hat-quiz` | 3 | cloudflare/workers-autoconfig(基线), pullh/1, main | 2 | 0 | 18 | 138385 (135.1 KB) | 否 | PR#1 open | 否 | 无（main 旧 .gitignore 已被基线分支更新覆盖） |
...
| `wendang11` | `wendang11` | 3 | main(基线), pullh/2, pullh/1 | 2 | 55 | 152 | 26262157 (25.05 MB) | 是（54 个 PR1 孤儿文件） | PR#1 closed(未并)、PR#2 closed(merged) | 是（skills-library 不解析，指向 turbine @ 736985ce2） | submodule skills-library 未解析（仅留 .gitmodules）；PR1 冲突版 README 另存 docs/legacy/README-pr1.md |
